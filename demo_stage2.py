    ...
    def demonstrate_embeddings(self):
        ...
        if DEPENDENCIES_AVAILABLE:
            try:
                ...
                embeddings = pipeline.embedding_generator.generate_embeddings(
                    sample_content)

                print(f"✅ Generated {len(embeddings)} embeddings")
                print(
                    f"📊 Embedding dimensions: {len(embeddings[0]) if embeddings else 'N/A'}")
                ...
                for topic in topics:
                    for card_type in card_types[:2]:  # Limit for demo
                        try:
                            ...
                            quality = generator.assess_content_quality(
                                card, card_type)
                            ...
                            print(
                                f"⚠️  Error generating {
                                    card_type.value} for {topic}: {e}")
