...
class WikipediaOERCrawler:
    ...
    def extract_article_content(self, title: str) -> Optional[OERResource]:
        ...
        try:
            ...
            has_examples = bool(re.search(r'\bexample\b|\bfor instance\b|\be\.g\.\b', 
                                        content_text, re.IGNORECASE))
            has_exercises = bool(re.search(r'\bexercise\b|\bpractice\b|\bproblem\b', 
                                         content_text, re.IGNORECASE))
    ...
    def _extract_topic_from_title(self, title: str) -> str:
        ...
        math_keywords = ['mathematics', 'algebra', 'calculus', 'geometry', 'statistics']
...
class KhanAcademyOERCrawler:
    ...
    def search_content(self, topic: str, content_type: str = "video") -> List[Dict]:
        """
        ...
        """
        # Placeholder implementation - would need to handle Khan Academy's structure
        search_results = [
            {
...
class OERContentAggregator:
    ...
    def crawl_topic(self, topic: str, max_resources: int = 20) -> List[OERResource]:
        """
        ...
        print(f"✓ Collected {len(collected_resources)} resources from Wikipedia")
    ...
    def find_similar_content(self, query: str, top_k: int = 5) -> List[Tuple[OERResource, float]]:
        """
    ...
    def get_content_statistics(self) -> Dict:
        ...
        avg_readability = sum(r.readability_score for r in self.resources) / total_resources
        ...
        for resource in self.resources:
            ...
            content_types[resource.content_type] = content_types.get(resource.content_type, 0) + 1
        ...
        return {
            ...
            'resources_with_examples': sum(1 for r in self.resources if r.has_examples),
            'resources_with_exercises': sum(1 for r in self.resources if r.has_exercises)
        }
...
class AsyncOERCrawler:
    ...
    async def _crawl_single_url(self, session: aiohttp.ClientSession, url: str) -> Optional[OERResource]:
