...
class ContentClusterer:
    ...
    def fit_predict(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        """
        self.logger.info(f"Clustering {len(embeddings)} embeddings into {self.n_clusters} clusters")
    ...
    def get_cluster_info(self, embeddings: np.ndarray, 
        ...
        for cluster_id in range(self.n_clusters):
            ...
            if cluster_size > 0:
                ...
                topics = [item.get('topic', 'unknown') for item in cluster_metadata]
                most_common_topic = max(set(topics), key=topics.count) if topics else 'unknown'
...
class EmbeddingPipeline:
    ...
    def process_content(self, content_list: List[Dict], 
        ...
        if cluster and len(content_list) >= 3:  # Need minimum items for clustering
            self.logger.info("Performing content clustering...")
            n_clusters = min(10, max(2, len(content_list) // 5))  # Adaptive cluster count
            self.clusterer = ContentClusterer(n_clusters=n_clusters)
            cluster_labels = self.clusterer.fit_predict(embeddings)
            cluster_info = self.clusterer.get_cluster_info(embeddings, content_list)
    ...
    def search_content(self, query: str, k: int = 5) -> List[Tuple[Dict, float]]:
        """
    ...
    def recommend_similar_content(self, content_index: int, 
        ...
        Returns:
            List of (content, similarity_score) tuples
        """
        if not self.search_engine or content_index >= len(self.search_engine.metadata):
            raise ValueError("Invalid content index or search engine not initialized")
