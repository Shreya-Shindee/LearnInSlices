...
class CollaborationModels:
    ...
    class PeerInteraction(Base):
        ...
        # Interaction details
        # explanation, hint, correction, encouragement
...
class CollaborationEngine:
    ...
    def leave_room(self, room_id: int, user_id: int) -> bool:
        ...
        if result.rowcount > 0:
            ...
            if room:
                room.current_participants = max(
                    0, room.current_participants - 1)
                room.last_activity = datetime.now(timezone.utc)
    ...
    def create_peer_challenge(self, challenger_id: int, challenged_id: int,
        ...
        self.logger.info(
            f"Challenge {
                challenge.id} created: {challenger_id} vs {challenged_id}")
        return challenge.id
    ...
    def get_study_rooms(
        ...
        query = self.db.query(CollaborationModels.StudyRoom)\
            .filter(CollaborationModels.StudyRoom.is_active)

        if topic:
            query = query.filter(
                CollaborationModels.StudyRoom.topic.ilike(
                    f"%{topic}%"))
    ...
    def _is_challenge_complete(self, challenge) -> bool:
        ...
        if challenge.time_limit_minutes:
            time_elapsed = (datetime.now(timezone.utc) -
                            challenge.started_at).total_seconds() / 60
    ...
    def get_user_collaboration_stats(self, user_id: int) -> Dict:
        ...
        group_memberships = self.db.query(
            CollaborationModels.StudyGroupMember) .filter(
            CollaborationModels.StudyGroupMember.user_id == user_id) .all()
