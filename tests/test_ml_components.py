...
class TestOERCrawler:
    ...
    def test_wikipedia_get_content(self, mock_get):
        ...
        content = self.wikipedia_crawler.get_content(
    ...
    def test_aggregator_collect_resources(
            self, mock_mit, mock_khan, mock_wiki):
...
class TestEmbeddings:
    ...
    def test_semantic_similarity(self):
        ...
        sim_1_2 = self.embedding_generator.calculate_similarity(
            embeddings[0], embeddings[1])
        sim_1_3 = self.embedding_generator.calculate_similarity(
    ...
    def test_content_clustering(self):
        ...
        cluster_texts = {i: [texts[idx] for idx in cluster]
    ...
    def test_embedding_pipeline(self):
        ...
        for resource in processed_resources:
            assert hasattr(
                resource,
                'embedding') or 'embedding' in resource.__dict__
...
class TestMicroCardGenerator:
    ...
    def test_content_quality_assessment(self):
        ...
        score = self.generator.assess_content_quality(
            good_card, CardType.CONCEPT)
        assert score > 0.5
...
class TestIntegration:
    ...
    def test_search_and_generate_workflow(self):
        ...
        embeddings = self.pipeline.embedding_generator.generate_embeddings(
            mock_content)
        ...
        assert len(cards) > 0
        assert all(card['difficulty_level'] ==
                   DifficultyLevel.BEGINNER for card in cards)
